          ...
          <div class="col-12">
            <div class="row" v-if="provideCart.data.length > 0">
              <div class="col-md-6 mb-md-0 mb-50">
                ...
                <hr class="bg-light my-0">
                <div class="input-group mt-40 mb-30" :class="{'pb-10': error.length > 0}">
                  ...
                  <input
                    ...
                    :placeholder="
                      (provideCart.total !== provideCart.final_total) ? '您已套用優惠券' : '套用優惠券'
                    "
                    v-model="code"
                  >
                  <div class="error fs-4 text-danger" v-if="error.length > 0">{{ error }}</div>
                  <button
                    class="btn btn-lg btn-primary fs-3"
                    type="button"
                    :disabled="provideCart.total !== provideCart.final_total || !code"
                    @click="useCoupon"
                  >套用優惠券</button>
                </div>
                <div class="row">
            ...
            <div class="row" v-else>
              <div class="col-md-12 mb-md-0 mb-50">
                <div class="d-flex flex-column align-items-center justify-content-between py-40">
                  購物車內沒有商品
                  <router-link
                    to="/products?category=all"
                    class="btn btn-primary d-inline-flex align-items-center mt-20"
                  >
                    <span class="material-icons-outlined me-5">chevron_left</span>
                    繼續購物
                  </router-link>
